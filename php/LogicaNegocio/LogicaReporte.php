<?php
    ...
    $IdDoc = $_POST['Docente'];
    $CantidadDeFilas = ValidarSiExisteEncuestaLogica($IdDoc);

    if($CantidadDeFilas > 0){
        $PromedioPorGrupo =PromedioPorGrupoLogica($Id_Docente);
        $Resultado = ConsultarObservacionesDocenteLogica($Id_Docente);
        $PromedioGeneral = PromedioGeneralDocenteLogica($Id_Docente);
        $DatosDocente = ObtenerDatosDocenteLogica($Id_Docente);
        ...
        class PDF extends FPDF{
            ...
            function TablaPromedioPorCurso($PromedioPorGrupo,$Resultado,$PromedioGeneral){
                $this->Ln();
                $this->Ln();
                $this->SetFillColor(31,78,120);
                $this->SetTextColor(255);
        ...
        echo "No hay encuestas para ese docente";
        die;
    }
?>